...
Grok 休息；剩餘身分與剩餘工程由 Luna 執行，見
[Luna local authorized playbook](product/luna-local-authorized-playbook.md)。
...
> ~~`WEB-P0-01/02/03`~~ privacy index/performance/axe（**2026-09-07／2026-09-08 完成，見下**）。
> 這些不是全面重開 Stage 0。booking write route、cloud Firestore 與 worker 多實例仍未授權。完整證據見
...
> `requiresDecision` fail-closed；D-003 未核准前 `/privacy` 維持 noindex。
>
> **`WEB-P0-02`／`WEB-P0-03` 已於 2026-09-08 完成（PR #83，merge `49efeda`）。**
> 該 SHA 的 `verify` run `34201616025` 含 performance／axe 證據。這不是 TW-05
> 真人輔具驗收。
>
...
核准，工程 C0～C6 synthetic slices 已在隔離專案完成，machine Canon 記錄
C1～C6 `deploymentAuthorities=granted`。這不解鎖 production、formal booking、
real data、backup/PITR production evidence 或新 cloud mutation；C1 isolated
foundation 本身仍不解鎖 production IdP、Firestore、backup/PITR 或 runtime。
