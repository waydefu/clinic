...
### 5.5 E2E 週檢視測試只在營業時間內會通過（已修）

2026-08-01 一個純文件的 PR 讓 CI 紅燈，4 個 `week-calendar.spec.ts` 測試失敗。
`main` 當時是綠的，變更也沒碰任何程式碼——**因為那是時鐘相依，不是變更造成的**。

| 時間（台北） | 結果 |
| --- | --- |
| 14:54 週六（營業中） | 通過 |
| 18:32 週六（已打烊） | 4 個失敗 |
| 18:35 本機重跑 | 相同 4 個失敗，確定性重現 |

機制：`admin-bootstrap.js` 的 `renderWeek()` 用「預約與時段中最早的那一筆」決定
顯示哪一週，而 `createBooking` 點的是「現在之後的第一個可選時段」。實測於 18:59：

```text
最早時段    2026-08-01T02:00Z = 今天 10:00 台北（過去的時段仍留在 state）
訂到的預約  2026-08-05T04:00Z = 下週三 12:00 台北
```

週檢視顯示本週、預約落在下週，於是 `[data-week-event]` 為 0。

修法是把種子預約**錨定到最早時段的那一天**——那一天依定義必然落在被渲染的週內，
與執行時刻無關。同時把日期改以台北時區計算，原本直接 `slice(0, 10)` 取 UTC 日期
會在跨日時取錯。修好後於 19:00（打烊後）重跑，6 個全過。

**這是本專案至今發現最嚴重的一種假綠燈**：它讓 CI 結果取決於牆上時鐘。任何在
夜間、或週日至週二（休診日）執行的 CI 都會失敗，而白天執行則看不出任何異常。
