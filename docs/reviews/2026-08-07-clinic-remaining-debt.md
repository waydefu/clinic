...
**措辭：** 這一輪之後才可以說「clamp 盲點已修復」。但 layout 類仍是 ratchet 而非
清零，要照實寫。
...
- **線上 WordPress 官網沒有動**（使用者 2026-08-07 指示不處理）。查證時看到的
  `alt=""` 與兩張只有像素的複雜資訊圖，仍記在
  [療程卡影像修復](2026-08-07-clinic-service-imagery.md)裡。
- **間距與容器的 clamp 端點沒有收斂**，只是進了 ratchet。收斂會動到版面呼吸感，
  而間距網格在 1.5→2→3→4rem 之間跳得很開，該補哪一階要先決定。
- **`typography.spec.ts` 沒有納入 WebKit**，理由見第四節。
- **200% 的人工驗收沒有做**——自動化的是 proxy。
