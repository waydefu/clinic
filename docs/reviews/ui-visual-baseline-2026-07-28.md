...
| 決策 gate | D-001～D-011 全為 `pending`；D-006 與 D-010 仍阻擋 Stage 2 | [decision register](../product/phase-1-decision-register.md) |
...
因此下一步不是直接啟用後端：先完成 Stage 1 具名 owner 決策；至少 D-006 與 D-010
核准後，才可提出 Stage 2 cloud foundation／staff identity。現階段可安全維護的仍是
local、Emulator、synthetic UI、文件、測試與不啟用新能力的設計。
