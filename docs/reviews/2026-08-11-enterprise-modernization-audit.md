...
| SCM-R05 | 修綠 dependency audit gate：`nanoid` 提升到 **`>=3.3.18`**（升 `vitest`/`vite`/`postcss` 或加 pinned override），並逐筆 triage 那 10 筆／Dependabot 9 筆；`SCM-006`；無依賴，是 `SCM-R01` 的前置。**2026-08-17 更正：** 本列原記 `>=3.3.17`，那是 2026-08-11 當下 advisory 的門檻；`GHSA-2v37-7h3g-55p8` 之後被修訂為 vulnerable `<3.3.18`／patched `>=3.3.18`（來源：commit `fc15bfd` 的 CI run `31994942617`）。**照舊值提到 `3.3.17` 不會讓 gate 轉綠。** 動手前先以當下的 `pnpm audit` 輸出確認門檻。 | AppSec/Maintainer；1–2 | 只改 manifest/lockfile，不動 runtime source；`git revert` 即回滾；驗收＝`Verification evidence` 與 dependency audit job 在同一 commit 轉綠，且每筆殘留 alert 有 owner／理由／到期日。 |
| ~~SCM-R01~~ **（2026-08-18 完成）** | Semgrep 進 required evidence＋修正已確認的 remote enforcement gap；**相依 `SCM-R05`** | AppSec/CI；1–2 | 故意失敗 PR；保留獨立 workflow；紅 SAST 不可 merge。**前置：aggregate 必須先為綠，否則分不出擋住 PR 的是 SAST 還是 audit，驗收無效。** — **已滿足並驗收：** 掃描實作抽成 `.github/workflows/sast-scan.yml`（`workflow_call`），`verify.yml` 在同一 run 內以 `sast` job 呼叫，`evidence` 依賴它；`sast.yml` 保留為每週排程／手動包裝，獨立可見性未失。對照組 PR #18（候選 `351e4034`、run `32100761005`）全綠；故意失敗 PR #19（候選 `d49330c8`、run `32101192719`）以 `clinic.javascript.weak-cryptography` 產生 1 筆 blocking finding，`sast` 與 `Verification evidence` 同時變紅、其餘十個 job 全綠、`mergeStateStatus=BLOCKED`、未用 admin bypass，已關閉未合併。branch protection 未變更。 |
