...
拆分後最慢的一組是 `e2e-appointments`，兩次 CI 分別為 **3m27s** 與 **3m47s**
（後者是 rebase 到含供應鏈修補的 `main` 之後、實際被合併的那一次）。共用 runner
的時間本來就會抖動，這裡記兩次而不是取一個好看的數字。
...
5. 拆成六組後總 CI 分鐘數上升——每組各自 build 並安裝 Chromium 一次。牆鐘時間
   改善（8m50s → 3m47s）是以此為代價換來的。若要收回，下一步是把 build 抽成前置
