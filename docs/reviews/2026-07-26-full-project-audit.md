...
**這不是工程能自行決定的事項**，已列為 D-012 的候選，等負責人對照官方注意事項
認定。在此之前不改動資產。
...
**本次未能確認實際狀態**（本機沒有 `gh`、沒有 token、Chrome 擴充未連線）。負責人
需執行該指令，或到 `Settings → Branches → main` 確認必要檢查含
