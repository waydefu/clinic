...
reschedule, complete, no-show and follow-up are on the same gate). CAL-PILOT is not a
...
| IP-001 internal-test booking | `POST /v1/bookings`, `GET /v1/bookings/:id`, `POST /v1/bookings/:id/cancel`, `POST /v1/bookings/:id/reschedule`, `POST /v1/bookings/:id/complete`, `POST /v1/bookings/:id/no-show`, `POST /v1/bookings/:id/follow-up`, `POST /v1/bookings/:id/delete` | Appointment command/query schemas and `RecordFollowUpRequestSchema` / `RecordFollowUpResponseSchema` | Fail-closed isolated-test reads/writes. Default 503. Follow-up is scheduling-only (`required` / `not_required` + target time); clinical notes, certificate count and case-manager assignment stay off this command. Not public production. |
