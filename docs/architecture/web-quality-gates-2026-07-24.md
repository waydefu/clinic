...
- **建置開始壓縮 CSS**（`scripts/build-web.mjs` 的 `minifyStylesheets`，用
  esbuild）。先前註解原封不動出貨，患者頁樣式表因此是 14.6 KiB、工作臺 21.3 KiB
  ——而這個專案的樣式表帶著大量解釋「為什麼這樣寫」的中文註解。等於在懲罰把理由
  寫清楚：註解愈完整，使用者付的錢愈多。壓縮後降到 6.8／11.1 KiB，樣式表預算
  跟著從 14／22 收到 10／16。
