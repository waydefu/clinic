...
- ~~患者頁仍有過多 14px 可見文字~~ **已於 2026-07-25 依語意重分級**：導覽、
  ...
  **2026-08-06 補記：同一件事在診所官網做了一次。** 官網當時有 32 處文字低於
  14px（最小 9.92px），因為它不在 `typography.spec.ts` 的範圍內——這裡的重分級
  做完之後，沒有人回頭問「還有哪一頁沒做」。收斂後官網 1440px 的相異字級從 20 種
  降到 6 種。細節見[介面規則書](ui-ux-rules.md) §7.1。
- **本文「字體家族只有 Regular 與 Bold 兩檔」這句話需要限定條件（2026-08-06）。**
  §10 的字重段落據此把 600 以上視為同一個字面。用 canvas 量墨水量實測本機
  （`font-synthesis: none`）得到的分組是 sans `400 | 500 | 600=650=700=750 | 800`、
  serif `400 | 500 | 600 | 650=700=750 | 800`——**650 與 750 確實是假階，但 500 與
  800 不是**。工作臺目前沒有用到 500／800，所以那段結論在工作臺上成立；官網用到了，
  合併它們會實際改變畫面。中文系統字體的字重供應本來就因平台而異，跨平台複核時
  要重量一次，不要把單一環境的量測寫成通則。
...
- 頁首**不再有寫死的整列元素**（2026-07-26 線上回報「留白過多」）。導覽先前是
  `width: 100%`，於是它必定獨佔一列、環境徽章也只能再排一列——導覽右邊與徽章
  左邊各是一塊空白。改成自然寬度＋`flex-shrink: 0`，由 flex 決定併列或換列：
  線上站導覽只有兩個連結（`內部工作臺` 是 local-only）148px＋徽章 171px＋12px
  欄距＝331px，塞得進 343px 於是併成同一列；本機三個連結 228px＋175px 放不下，
  自己掉回兩列；320px 兩種設定都換列。**寧可換列也不讓導覽橫捲**，所以不可壓縮。
  併列時徽章 `align-self: center`（22px 徽章對 44px 觸控列，容器是 flex-start，
  不置中會差 11px）。同時 `row-gap` 12→8px、`column-gap` 24→12px、頁首內距
  12→8px。線上頁首 212→**162px**，第一個問題 502→**452px**；本機 212→192px。
