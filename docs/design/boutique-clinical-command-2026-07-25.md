...
- 工作臺品牌列在 ≤30rem 仍保留標誌、中文診所名稱與英文副標；以較小欄距、
  `min-width: 0` 與英文換行吸收窄幅，不再隱藏整個品牌文字區。
- 患者頁品牌列在 ≤48rem 同樣保留英文副標（先前只修了工作臺那一份，患者頁——
  也就是對外的那一面——仍在隱藏）。副標疊在標誌 40px 高度內，並不使頁首長高；
  先前用它換來的是寬度，好讓環境徽章併上品牌列。改為讓徽章自己換一列，代價由
  頁首的**縱向節奏**吸收：手機把單列折成四列時，桌機 24px 的 `gap` 會變成 72px
  的垂直留白，因此 `column-gap` 維持 24px、`row-gap` 收到 12px。淨結果是頁首
  248→212px，患者看到第一個問題的位置由 592px 上移到 502px（比修改前更好）。
