...
7. 回診只能呈現授權人員的明確決定；系統與 AI 不得自行做臨床判斷。回診項目可複選，並可記錄診斷書份數。目標為「日期＋時間」，僅限營業日的回診可掛號網格；未營業日期會即時提示且無法儲存。確認「需要回診」後會產生一筆回診提醒的日曆投影（上日曆），並在櫃台處理清單顯示「待安排回診」，供櫃台安排下次門診時段；改為「目前無需回診」則移除該投影。
...
| 管理者專屬回診表單對櫃台完全不渲染；完成到診後只有管理者會被帶到回診決定 | 「看得到但存不了」不是權限提示，而是不完整的流程 | `renderSession` 的 `[data-admin-content]` 權限揭露 |
