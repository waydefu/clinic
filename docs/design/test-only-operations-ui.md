...
7. 回診只能呈現授權人員的明確決定；系統與 AI 不得自行做臨床判斷。回診項目可複選，並可記錄診斷書份數。目標為「日期＋時間」，僅限營業日的回診可掛號網格；未營業日期會即時提示且無法儲存。
...
| 預約以**週檢視**呈現：左時間軸＋週一～週日七欄，初診（左半欄）與回診（右半欄）並行雙色，休診日斜線，可週導覽，點事件捲到清單卡片 | 時間軸總覽比平鋪清單更能一眼看出當日排程；初診／回診並行是診所的人力線模型 | `modules/week-view.js` |
...
| 手機（≤48rem）篩選列用 2 欄格線、奇數個時最後一項跨滿 | 三個全寬下拉會佔滿整個畫面（2026-07-22 實機回報） | `workbench.css` 的 `.filter-bar` 媒體查詢 |
...
2026-07-23 於本機實機驗證：預約週檢視（初診左半欄／回診右半欄兩條並行線、時間定位正確、休診日斜線、週導覽、點事件捲到清單卡片並 flash、深色主題適配、375px 內部橫捲不溢出、0 console 錯誤）；日曆投影死信補回入口（模擬失敗→死信→重新排入，稽核鏈完整）。發現並修正：CSP `style-src 'self'` 會擋掉 inline `style` 屬性，週檢視改以 `data-*`＋CSSOM 定位。
