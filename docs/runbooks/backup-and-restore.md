...
**以下仍是待 D-010 核准的設計值。** 2026-07-27 問卷已記錄整體候選目標為
RPO 1 小時／RTO 4 小時，但尚無具名核准，也沒有 cloud 演練證明可達成。這不是
工程師能單方面決定的——RTO 越短，成本越高，而「多久沒有預約系統，診所還能
運作」只有診所答得出來。
...
| 整個 project 不可用 | **1 小時候選值，目前設計未證明** | **4 小時候選值，目前設計未證明** | 每日備份不足以單獨達標；須由 D-010 補跨 project 復原設計與演練 |
| 區域性故障 | **1 小時候選值，目前單區設計未達** | **4 小時候選值，目前未定義路徑** | 目前是單區設計，見 §7 |
...
- **跨區災難復原**：目前是單區設計；§2 雖記錄整體 RTO 4 小時的候選目標，
  區域故障的實際復原路徑仍未定義。跨區複寫的成本在診所規模下不一定合理，
  要由 D-010 核准目標與成本後再決定。
