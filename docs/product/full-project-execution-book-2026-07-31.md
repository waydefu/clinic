...
每一道 gate 的結果只能使用 `PASS`、`FAIL`、`NOT_RUN` 或 `UNAVAILABLE`。`NOT_RUN`
必須說明為何未執行與誰能授權執行；`UNAVAILABLE` 必須說明無法存取的系統、查證日期與
後續 owner。不得把「未發現失敗」、「過去曾通過」或「文件宣稱有 gate」寫成當前
`PASS`。動態證據還必須綁定查證時間、commit、環境與覆蓋範圍；缺任一項即標為
`UNVERIFIED`，不能推論遠端部署、required check 或 production 現況。

| 段落 | 內容 | 常見的寫壞方式 |
| --- | --- | --- |
| 一句話 | 這個階段到底做了什麼 | 寫成待辦清單而不是已完成的事 |
| 交付的修訂版本 | 分支、PR、每個 commit 與 merge commit | 只寫「已合併」，沒有 hash |
| 驗收證據 | 每道關卡的狀態、**實際數字**、commit／環境、CI 連結與 artifact SHA-256 | 寫「全部通過」，沒有狀態、檔數、測試數、掃描規則數或雜湊 |
| 稽核範圍與未覆蓋面 | 本次讀到的 repository／backend／database／cloud／production 範圍，以及無法存取的部分 | 把靜態 code review 寫成全面 production 稽核 |
...
| 下一位從這裡開始 | 有順序的待辦、可執行的 Roadmap ID 與必要核准，每項標明卡在哪 | 寫「請參考其他文件」或未指定下一個授權範圍 |
...
分支／PR／merge commit：
稽核範圍與明確未覆蓋面：
每道關卡的 PASS／FAIL／NOT_RUN／UNAVAILABLE（含數字、原因、commit、環境與連結）：
交付 artifact 的路徑與 SHA-256：
發現並修好的缺陷：
未處理、未跑或繞過的事項：
本機環境陷阱：
本階段記錄的決策與剩餘風險：
下一位接手者的第一步與下一個已授權／待核准 Roadmap ID：
目前 Stage 位置是否改變：
```
