...
export function saveAnnouncement(state, input) {
  state.workspace.announcement = {
    status: input.status === 'draft' ? 'draft' : 'published',
    title: safeText(input.title, '公告標題', 40),
    body: safeText(input.body, '公告內容', 160),
    updatedAt: new Date().toISOString(),
    updatedBy: currentAccount(state)?.id
  };
}
export function saveMaintenance(state, input) {
  state.workspace.maintenance = {
    enabled: input.enabled === true,
    title: safeText(input.title, '維護標題', 40),
    body: safeText(input.body, '維護說明', 160),
    startsAt: typeof input.startsAt === 'string' ? input.startsAt : '',
    resumeAt: typeof input.resumeAt === 'string' ? input.resumeAt : '',
    updatedAt: new Date().toISOString(),
    updatedBy: currentAccount(state)?.id
  };
}
export function isMaintenanceActive(maintenance, now = new Date()) {
  if (maintenance.enabled !== true) return false;
  const starts = maintenance.startsAt
    ? new Date(`${maintenance.startsAt}:00+08:00`)
    : undefined;
  const resume = maintenance.resumeAt
    ? new Date(`${maintenance.resumeAt}:00+08:00`)
    : undefined;
  if (starts !== undefined && now < starts) return false;
  if (resume !== undefined && now >= resume) return false;
  return true;
}
export function addRelease(state, input) {
  const nextId = `release_test_${String(state.workspace.releases.length + 1).padStart(3, '0')}`;
  state.workspace.releases.unshift({
    id: nextId,
    version: safeText(input.version, '版本', 24),
    summary: safeText(input.summary, '發布摘要', 120),
    publishedAt: new Date().toISOString(),
    publishedBy: currentAccount(state)?.id
  });
}
