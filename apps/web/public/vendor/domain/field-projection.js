...
export function projectRecordFields(role, record) {
    if (role === undefined || !isRole(role))
        return {};
    const projected = {};
    for (const [key, value] of Object.entries(record)) {
        if (MONEY_FIELD_NAMES.has(key) || CLINICAL_FIELD_NAMES.has(key))
            continue;
        projected[key] = value;
    }
    return projected;
}
export function isBlockedProjectionField(fieldName) {
    return (MONEY_FIELD_NAMES.has(fieldName) || CLINICAL_FIELD_NAMES.has(fieldName));
}
