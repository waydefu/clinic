...
function decodeBase32Hex(text) {
    ...
    for (const character of text) {
        ...
        if (value < 0)
            throw new DomainError('INVALID_VALUE', `"${character}" is not a base32hex character.`);
 ...
 * 隨機值的實作。
 */
export function toCalendarEventId(logicalKey) {
    if (typeof logicalKey !== 'string' || logicalKey.length === 0)
        throw new DomainError('INVALID_VALUE', 'logicalKey must be a non-empty string.');
    const eventId = encodeBase32Hex(new TextEncoder().encode(logicalKey));
    // 防禦性檢查：邏輯鍵短到編不出 5 個字元時，寧可在本機炸掉，也不要等到
    // 上線後被 Calendar API 退件。
    if (eventId.length < MIN_CALENDAR_EVENT_ID_LENGTH ||
        eventId.length > MAX_CALENDAR_EVENT_ID_LENGTH)
        throw new DomainError('INVALID_VALUE', `A Calendar event ID must be ${MIN_CALENDAR_EVENT_ID_LENGTH}-${MAX_CALENDAR_EVENT_ID_LENGTH} characters; got ${eventId.length}.`);
    return eventId;
}
/** 還原邏輯鍵，供人工追查與 runbook 使用。 */
export function fromCalendarEventId(eventId) {
    if (!isCalendarEventId(eventId))
        throw new DomainError('INVALID_VALUE', 'eventId is not a valid Calendar event ID.');
 ...
 */
export function isCalendarEventId(value) {
 ...
 */
export function calendarEventIdForStatus(appointmentId, status) {
 ...
 */
export function calendarEventIdForReschedule(appointmentId, targetSlotId) {
    return toCalendarEventId(`calendar_rescheduled_${appointmentId}_${targetSlotId}`);
}
