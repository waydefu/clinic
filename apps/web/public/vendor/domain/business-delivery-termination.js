...
function assertPolicy(policy) {
    if (!Number.isInteger(policy.minimumNoticeDays) ||
        policy.minimumNoticeDays < 1) {
        throw new DomainError('INVALID_VALUE', 'minimumNoticeDays must be a positive integer.');
    }
    if (!Number.isInteger(policy.controlledCopyRetentionDays) ||
        policy.controlledCopyRetentionDays < 1) {
        throw new DomainError('INVALID_VALUE', 'controlledCopyRetentionDays must be a positive integer.');
    }
}
...
function assertProof(proof) {
    if (!proof.authorized || !proof.reauthenticated) {
        throw new DomainError('DELEGATION_NOT_AUTHORIZED', 'termination authorization and re-authentication are required.');
    }
    assertOpaque(proof.scopeId, 'termination.scopeId');
    assertOpaque(proof.requestId, 'termination.requestId');
    assertOpaque(proof.authorizationReference, 'termination.authorizationReference');
    assertOpaque(proof.reauthenticationReference, 'termination.reauthenticationReference');
}
function assertNotice(input) {
    ...
    if (noticeStartedAt === null || noticeDueAt === null) {
        throw new DomainError('INVALID_VALUE', 'termination notice must retain both start and due timestamps.');
    }
    assertUtcTimestamp(noticeStartedAt, 'noticeStartedAt');
    assertUtcTimestamp(noticeDueAt, 'noticeDueAt');
    const expectedDueAt = addUtcDays(noticeStartedAt, input.policy.minimumNoticeDays);
    if (noticeDueAt !== expectedDueAt) {
        throw new DomainError('INVALID_VALUE', 'noticeDueAt does not match the original notice boundary.');
    }
    return {
        noticeStartedAt,
        noticeDueAt
...
function assertReturnEvidence(input) {
    if (input.returnManifestReference === null ||
        input.recipientConfirmationReference === null ||
        input.returnCompletedAt === null) {
        throw new DomainError('INVALID_VALUE', 'termination return requires a manifest, recipient confirmation, and completion timestamp.');
    }
    assertOpaque(input.returnManifestReference, 'returnManifestReference');
    assertOpaque(input.recipientConfirmationReference, 'recipientConfirmationReference');
    assertUtcTimestamp(input.returnCompletedAt, 'returnCompletedAt');
}
function assertControlledRetention(input) {
    if (input.returnCompletedAt === null ||
        input.controlledRetentionUntil === null) {
        throw new DomainError('INVALID_VALUE', 'controlled retention requires a completed return and expiry boundary.');
    }
    assertUtcTimestamp(input.returnCompletedAt, 'returnCompletedAt');
    assertUtcTimestamp(input.controlledRetentionUntil, 'controlledRetentionUntil');
    const expectedUntil = addUtcDays(input.returnCompletedAt, input.policy.controlledCopyRetentionDays);
    if (input.controlledRetentionUntil !== expectedUntil) {
        throw new DomainError('INVALID_VALUE', 'controlledRetentionUntil does not match the original retention boundary.');
    }
}
...
export function planBusinessTerminationOperation(input) {
    ...
    if (input.proof.requestId !== input.requestId) {
        throw new DomainError('INVALID_VALUE', 'termination request references must agree.');
    }
    ...
    const recipientConfirmationReference = input.recipientConfirmationReference ?? null;
    ...
    if (input.operation === 'open_notice') {
        if (input.state === 'active') {
            ...
            return basePlan({
                ...
                noticeDueAt: addUtcDays(originalNoticeStartedAt, input.policy.minimumNoticeDays),
        ...
        throw new DomainError('INVALID_VALUE', 'a termination notice cannot be reopened after return processing.');
    }
    if (input.state === 'active') {
        throw new DomainError('INVALID_VALUE', `${input.operation} requires an opened termination notice.`);
    }
    ...
    if (input.operation === 'record_return') {
        if (input.state === 'termination_pending') {
            if (Date.parse(input.nowAt) < Date.parse(verifiedNotice.noticeDueAt) &&
                noticeExceptionReference === null) {
                throw new DomainError('INVALID_VALUE', 'minimum notice period has not elapsed and no approved exception reference was supplied.');
            }
            ...
            if (Date.parse(completedAt) < Date.parse(verifiedNotice.noticeStartedAt)) {
                throw new DomainError('INVALID_TIMESTAMP', 'returnCompletedAt cannot precede noticeStartedAt.');
            }
            return basePlan({
                ...common,
        ...
        throw new DomainError('INVALID_VALUE', 'return can only be recorded before controlled retention begins.');
    }
    assertReturnEvidence({
        returnManifestReference,
        recipientConfirmationReference,
    ...
    if (verifiedReturnCompletedAt === null) {
        throw new DomainError('INVALID_VALUE', 'termination return completion evidence is required.');
    }
    if (input.operation === 'start_controlled_retention') {
        if (input.state === 'returned') {
            const retentionUntil = controlledRetentionUntil ??
                addUtcDays(verifiedReturnCompletedAt, input.policy.controlledCopyRetentionDays);
        ...
        throw new DomainError('INVALID_VALUE', 'controlled retention requires a completed return.');
    }
    if (input.state !== 'controlled_retention' &&
        input.state !== 'manual_close_review') {
        throw new DomainError('INVALID_VALUE', 'manual closure review requires controlled retention.');
    }
    assertControlledRetention({
        returnCompletedAt: verifiedReturnCompletedAt,
        controlledRetentionUntil,
    ...
    if (controlledRetentionUntil === null ||
        Date.parse(input.nowAt) < Date.parse(controlledRetentionUntil)) {
        throw new DomainError('INVALID_VALUE', 'controlled retention has not expired.');
    }
    if (input.backupDispositionConfirmed !== true ||
        input.auditDispositionConfirmed !== true) {
        throw new DomainError('INVALID_VALUE', 'backup and audit disposition evidence is required before manual closure review.');
    }
    return basePlan({
        ...common,
        operation: 'request_manual_close_review',
        resultingState: 'manual_close_review',
        idempotency: input.state === 'manual_close_review'
            ? 'already_applied'
            : 'new_operation'
    });
}
