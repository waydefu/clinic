 ...
 */
export const BUSINESS_DELIVERY_TIME_ZONE = 'Asia/Taipei';
...
function parseTaipeiDate(value) {
    ...
    if (month < 1 || month > 12 || day < 1 || day > daysInMonth(year, month)) {
        throw new DomainError('INVALID_VALUE', 'must be a real Taipei calendar date.');
    }
    return { year, month, day };
}
function formatTaipeiDate(year, month, day) {
    return [year, month, day]
        .map((part) => String(part).padStart(2, '0'))
        .join('-');
}
function addTaipeiCalendarDays(value, days) {
    const date = parseTaipeiDate(value);
    const shifted = new Date(Date.UTC(date.year, date.month - 1, date.day + days));
    return shifted.toISOString().slice(0, 10);
}
function addTaipeiCalendarMonthsClamped(value, months) {
    const date = parseTaipeiDate(value);
...
function taipeiCalendarDate(isoUtc) {
...
function taipeiDateStartAt(date) {
...
function assertNonNegativeInteger(value, fieldName) {
    if (!Number.isInteger(value) || value < 0) {
        throw new DomainError('INVALID_VALUE', `${fieldName} must be a non-negative integer.`);
    }
}
function assertChronological(at, startAt, endAt) {
    assertUtcTimestamp(at, 'earlyEnd.endedAt');
    const atMs = Date.parse(at);
    if (atMs < Date.parse(startAt) || atMs >= Date.parse(endAt)) {
        throw new DomainError('INVALID_VALUE', 'earlyEnd.endedAt must be inside the scheduled trial period.');
    }
}
/**
 ...
 */
export function selectFirstEligibleUsage(events) {
    ...
    if (!first) {
        throw new DomainError('INVALID_VALUE', 'A maintenance or test account cannot start the business trial.');
    }
    return first;
}
/**
...
export function calculateBusinessDeliveryMilestones(input) {
    ...
    if (firstEligibleUse.actorKind !== 'real_staff') {
        throw new DomainError('INVALID_VALUE', 'Only a real staff use may start the business trial.');
    }
    assertUtcTimestamp(firstEligibleUse.occurredAt, 'firstEligibleUse.occurredAt');
    ...
    assertPositiveInteger(policy.formalOperationCalendarMonths, 'formalOperationCalendarMonths');
    ...
    if (adjustmentDays > policy.maxAdjustmentDays) {
        throw new DomainError('INVALID_VALUE', 'adjustmentDays exceeds the approved maximum.');
    }
    const firstEligibleUseDate = taipeiCalendarDate(firstEligibleUse.occurredAt);
    const trialEndExclusiveDate = addTaipeiCalendarDays(firstEligibleUseDate, policy.trialCalendarDays);
    const adjustedTrialEndExclusiveDate = addTaipeiCalendarDays(trialEndExclusiveDate, adjustmentDays);
    const formalOperationCheckpointDate = addTaipeiCalendarMonthsClamped(firstEligibleUseDate, policy.formalOperationCalendarMonths);
    const trialEndExclusiveAt = taipeiDateStartAt(trialEndExclusiveDate);
    const adjustedTrialEndExclusiveAt = taipeiDateStartAt(adjustedTrialEndExclusiveDate);
    const formalOperationCheckpointAt = taipeiDateStartAt(formalOperationCheckpointDate);
    if (input.earlyEnd) {
        assertChronological(input.earlyEnd.endedAt, firstEligibleUse.occurredAt, adjustedTrialEndExclusiveAt);
        if (!OPAQUE_REFERENCE.test(input.earlyEnd.confirmationReference)) {
            throw new DomainError('INVALID_VALUE', 'earlyEnd.confirmationReference must be an opaque audit reference.');
        }
    }
    return {
        timeZone: BUSINESS_DELIVERY_TIME_ZONE,
