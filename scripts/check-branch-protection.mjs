...
/**
 * 純核心：把兩支 API 的回應收斂成「目前實際被強制的檢查」。匯出以便測試——
 * 這道 gate 判斷的是「紅燈能不能被合併」，讀錯任何一種設定形狀都會讓它誤報安全。
 *
 * @returns { enforced: Set<string> } 或 { unreadable: number } 代表無法判定
 */
export function enforcedChecks({ protection, rulesets }) {
  ...
  if (protection.status === 200) {
    ...
    return { unreadable: protection.status };
  }
...
export function missingChecks(enforced, required = REQUIRED_CHECKS) {
...
async function main() {
  ...
  const evaluated = enforcedChecks({ protection, rulesets });
  if (evaluated.unreadable !== undefined) {
    console.error(
      `Reading branch protection failed with HTTP ${evaluated.unreadable}.` +
