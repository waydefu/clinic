...
export const PRODUCTION_CALENDAR_INBOUND = 'GO_LIVE_DEFERRED' as const;
export const INTERNAL_PREPRODUCTION_COMPLETE = 'FAIL' as const;
export const PUBLIC_PRODUCTION_LAUNCH = 'DEFERRED' as const;
export const HUMAN_NOTIFICATION_PATH_STATUS =
  'IMPLEMENTED_NOT_DEPLOYED' as const;
export const HUMAN_NOTIFICATION_PROVEN = 'HUMAN_NOTIFICATION_PROVEN' as const;
...
export type ObservabilitySignal =
  | 'http_5xx'
  | 'booking_write_failure'
  | 'booking_transaction_failure'
  | 'return_lookup_rate_limited'
  | 'auth_failure'
  | 'authz_denial'
  | 'outbox_backlog'
  | 'outbox_oldest_age'
...
export type AlertSeverity = 'immediate' | 'weekday';

export type WpB4ImmediateAlert =
...
export interface AlertPolicyDefinition {
  readonly id: WpB4ImmediateAlert;
  readonly severity: 'immediate';
  readonly signal: ObservabilitySignal;
  readonly threshold: number;
  readonly windowMs: number;
  readonly comparison: 'gte';
  readonly notificationPath: readonly [
    'alert_policy',
...
export const WP_B4_IMMEDIATE_ALERTS: readonly AlertPolicyDefinition[] =
  Object.freeze([
    {
...
export type ProbeResult = 'ok' | 'unavailable' | 'not_probed';

export interface OperationalHealthInput {
  readonly processAlive: boolean;
  readonly firestore: ProbeResult;
  readonly calendarAdapter: ProbeResult;
  readonly requiredConfigPresent: boolean;
  readonly bookingGateEnabled: boolean;
  readonly outboxDeadLetterCount: number;
  readonly outboxOldestAgeSeconds: number;
  readonly candidateBacklog: number;
  readonly calendarSyncStale: boolean;
  readonly calendarGoneRecoveryNeeded: boolean;
  readonly backupFailed: boolean;
  readonly iamAlertIntegrated: boolean;
  readonly workerDegraded: boolean;
}
...
export interface OperationalCheck {
  readonly id: string;
  readonly status: 'ok' | 'degraded' | 'unhealthy' | 'disabled' | 'unknown';
}

export interface OperationalHealthEvaluation {
  readonly status: OperationalStatus;
  readonly readiness: Readiness;
  readonly liveness: 'alive' | 'dead';
  readonly checks: readonly OperationalCheck[];
  readonly firingAlerts: readonly WpB4ImmediateAlert[];
}
...
export function evaluateOperationalHealth(
  ...
  const checks: OperationalCheck[] = [
    ...
    check(
      'outbox_dead_letter',
      input.outboxDeadLetterCount > 0 ? 'degraded' : 'ok'
    ),
    check(
      'outbox_age',
      input.outboxOldestAgeSeconds >= OUTBOX_AGE_ALERT_SECONDS
        ? 'degraded'
        : 'ok'
    ),
    check(
      'candidate_backlog',
      input.candidateBacklog >= CANDIDATE_BACKLOG_ALERT_THRESHOLD
        ? 'degraded'
        : 'ok'
    ),
    check(
      'calendar_sync',
      input.calendarSyncStale || input.calendarGoneRecoveryNeeded
        ? 'degraded'
        : 'ok'
    ),
    check('backup', input.backupFailed ? 'unhealthy' : 'ok'),
    check(
      'iam_alert_integration',
      input.iamAlertIntegrated ? 'ok' : 'degraded'
    ),
    check('worker', input.workerDegraded ? 'degraded' : 'ok')
  ];

  const liveness: 'alive' | 'dead' = input.processAlive ? 'alive' : 'dead';
  ...
  const status: OperationalStatus = !input.processAlive
    ? 'unhealthy'
    : unhealthy
      ? 'unhealthy'
      : degraded
...
export interface SignalCounts {
  ...
  readonly bookingWriteFailure: number;
  readonly bookingTransactionFailure: number;
  readonly authFailure: number;
  readonly authzDenial: number;
  readonly returnLookupRateLimited: number;
  readonly deadLetter: number;
  readonly outboxOldestAgeSeconds: number;
  readonly backupFailure: number;
  readonly iamSetIamPolicy: number;
}

export function firingImmediateAlerts(
  input: OperationalHealthInput,
  counts?: SignalCounts
): WpB4ImmediateAlert[] {
  const firing: WpB4ImmediateAlert[] = [];
  ...
  if (counts && counts.iamSetIamPolicy >= 1) firing.push('iam_setiampolicy');
...
export interface HumanNotificationPath {
  readonly status: typeof HUMAN_NOTIFICATION_PATH_STATUS;
  readonly channelKind: 'email';
  readonly transport: 'pubsub_plus_email';
  readonly recipientSource: 'secret_or_tfvar_never_in_repo';
  readonly pubsubTopic: 'c1-application-alerts';
  readonly proven: false;
}

export const HUMAN_NOTIFICATION_PATH: HumanNotificationPath = Object.freeze({
...
export interface StructuredLog {
  ...
  readonly result: 'ok' | 'error' | 'denied' | 'rate_limited';
  readonly errorCode: string | null;
  readonly durationMs: number;
  readonly retryState: 'none' | 'retrying' | 'dead_lettered';
}

const OPAQUE = /^[A-Za-z0-9_-]{1,128}$/;
const ENVIRONMENT = /^[A-Za-z0-9_-]{1,64}$/;

export function assertSafeMetricLabels(
  labels: Readonly<Record<string, string>>
): void {
  for (const [key, value] of Object.entries(labels)) {
    if (
      FORBIDDEN_METRIC_LABELS.some(
...
export function sanitizeStructuredLog(input: StructuredLog): StructuredLog {
  for (const key of Object.keys(input)) {
    if (
      FORBIDDEN_LOG_KEYS.some(
        (forbidden) => forbidden.toLowerCase() === key.toLowerCase()
      )
    ) {
...
export interface WeekdayOperationalSummary {
  readonly kind: 'weekday_operational_summary';
  readonly environment: string;
  readonly generatedAt: string;
  readonly synthetic: true;
  readonly rateLimited: number;
  readonly authDenials: number;
  readonly authzDenials: number;
  readonly retries: number;
  readonly recoveries: number;
  readonly outboxBacklog: number;
  readonly calendarCandidateBacklog: number;
  readonly apiP95Ms: number | null;
  readonly resourceCostSignal: string | null;
}

export function renderWeekdayOperationalSummary(
  input: Omit<WeekdayOperationalSummary, 'kind' | 'synthetic'>
): WeekdayOperationalSummary {
  return {
...
export interface FailClosedApiEvaluation {
  readonly ok: boolean;
  ...
  if (method === 'POST' && path === '/v1/bookings') {
    ...
    if (status === 401 || status === 403) {
      return {
        ...
        status,
        reason:
          'accountless-booking-regressed: public create must not require login'
      };
    }
    if (status >= 200 && status < 300) {
...
export function routeFamilyFromPath(path: string): SafeRouteFamily {
  if (path.includes('/health')) return 'health';
  if (path.includes('/return-lookup')) return 'return_lookup';
  if (path.includes('/bookings')) return 'public_booking';
  if (path.includes('/calendar')) return 'calendar';
  if (path.includes('/schedule') || path.includes('/slots')) return 'schedule';
  if (path.includes('/calendar-session')) return 'staff';
  return 'other';
}
...
export function candidateBacklogExceeded(count: number): boolean {
