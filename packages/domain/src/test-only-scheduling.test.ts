...
describe('test-only schedule rules', () => {
  it('sorts valid weekly rules and date exceptions without changing their meaning', () => {
    ...
    expect(result.weeklyAvailability.map((entry) => entry.weekday)).toEqual([1, 2]);
  ...
  it('rejects an invalid weekday, inverted time, overlapping windows and duplicate exceptions', () => {
    expectInvalid(() => createTestOnlySchedule({ ...schedule, weeklyAvailability: [{ weekday: 7, intervals: [{ startLocalTime: '09:00', endLocalTime: '10:00' }] }] }));
    expectInvalid(() => createTestOnlySchedule({ ...schedule, weeklyAvailability: [{ weekday: 1, intervals: [{ startLocalTime: '10:00', endLocalTime: '09:00' }] }] }));
    expectInvalid(() => createTestOnlySchedule({ ...schedule, weeklyAvailability: [{ weekday: 1, intervals: [{ startLocalTime: '09:00', endLocalTime: '11:00' }, { startLocalTime: '10:00', endLocalTime: '12:00' }] }] }));
    expectInvalid(() => createTestOnlySchedule({ ...schedule, dateExceptions: [{ date: '2030-01-07', kind: 'closed', reasonCode: 'REST_DAY' }, { date: '2030-01-07', kind: 'closed', reasonCode: 'REST_DAY' }] }));
  });
});
