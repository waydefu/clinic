 ...
 * 患者身分的單一規則來源。
 *
 * 先前這組規則只存在於瀏覽器（`apps/web/public/modules/patient-registry.js`），
 * domain 完全沒有患者身分的概念——`planBooking` 只收一個現成的 `patientId`。
 * 那代表 API 真的上線時，同一組規則得再寫一次，而事實上的規格會是那份不會出貨
 * 的瀏覽器程式。這個檔案把規則搬到兩端共用的位置；瀏覽器改為載入 vendored 副本
 * （ADR-0004），只保留狀態存取與中文訊息。
 *
 * **這裡不決定政策**：保存期限、法律依據與正式的身分模型是 D-001…D-003。
 * 這個模組只回答「這組輸入的格式是否成立，以及同一個人怎麼認得出來」。
 */
...
export type PatientIdentityField =
  ...
  | 'nationalId'
  | 'passportNumber'
  /** 「身分證與護照至少要有一個」——問題不屬於任何一個欄位，而是兩者之間。 */
  | 'identityDocument';

/**
 * 失敗原因用代碼表示，不用訊息。
 *
 * domain 不做在地化：同一個 `format` 在患者表單要說「格式為 1 個英文字母加
 * 9 位數字」，在未來的 API 錯誤裡則是 v1 envelope 的欄位錯誤。把中文字串放進
 * domain 會讓這一層綁死在單一介面上。
 */
...
function birthDateIssue(
  ...
  if (!CALENDAR_DATE_PATTERN.test(value)) return 'format';
  if (!isCalendarDate(value)) return 'not_a_calendar_date';
  ...
  const startOfDay = new Date(
    `${value}T00:00:00${TAIPEI_UTC_OFFSET}`
  ).getTime();
  if (startOfDay > now) return 'in_the_future';
  return undefined;
}
...
export function patientIdentityKey(input: PatientIdentityInput): string {
  const nationalId = text(input.nationalId);
  if (nationalId !== '') return `id:${nationalId.toUpperCase()}`;
  const passportNumber = text(input.passportNumber);
 ...
 */
export function maskIdentityDocument(input: PatientIdentityInput): string {
  const nationalId = text(input.nationalId);
