 ...
 * 容量是以人力線計算，不是以時鐘分鐘計算。
 */
export const SLOT_MINUTE_MARKS: Readonly<
  Record<BookingKind, readonly number[]>
> = Object.freeze({
  initial: Object.freeze([0, 30]),
  follow_up: Object.freeze([15, 45])
});

const BOOKING_KINDS: readonly BookingKind[] = ['initial', 'follow_up'];
...
export interface DateException {
  ...
  readonly kind: 'closed' | 'extra_open';
  readonly intervals?: readonly TimeInterval[];
}

/** 醫師的固定行程；這些時鐘時間不開放掛號，各自屬於一個網格。 */
export type BlockedTimes = Readonly<Record<BookingKind, readonly string[]>>;

export interface Schedule {
  readonly timeZone: string;
  readonly weeklyAvailability: readonly WeeklyAvailability[];
  readonly dateExceptions: readonly DateException[];
  readonly blockedTimes?: Partial<BlockedTimes>;
}

export interface SlotGenerationOptions {
  /** 產生時段的第一天（YYYY-MM-DD，台北時間）。 */
  readonly startDate: string;
  readonly dayCount: number;
  readonly durationMinutes?: number;
}

/** 已發布排班的版本狀態。發布時用來偵測「兩人同時發布」。 */
export interface SchedulePublicationState {
  readonly publishedVersion: number;
  readonly publishedAt: string | null;
}

/** 排班發布只需要知道預約壓在哪一格、是否還未結束。 */
...
export interface SchedulePublicationRequest {
  ...
  /**
   * 發布者以為自己接在哪一版之後。與現況不符即代表有人先發布了，必須擋下
   * 而不是覆蓋——排班是所有可預約時段的來源，靜默覆蓋會讓另一位管理者的
   * 變更消失無蹤。
   */
  readonly expectedVersion: number;
  readonly slotGeneration: SlotGenerationOptions;
  readonly audit: AuditContext;
  readonly requestedAt: string;
  readonly idempotency: IdempotencyContext;
}

export interface SchedulePublicationPlan {
  readonly publishedVersion: number;
...
function isArrayShaped(value: unknown): boolean {
...
function localIso(date: string, totalMinutes: number): string {
  return new Date(
    `${date}T${clockText(totalMinutes)}:00${CLINIC_UTC_OFFSET}`
  ).toISOString();
}
...
function assertIntervals(intervals: readonly TimeInterval[] | undefined): void {
  ...
  if (intervals === undefined || intervals.length === 0) {
    throw new DomainError(
      'INVALID_VALUE',
      'A day needs at least one interval.'
    );
  }
  ...
  for (const [index, interval] of sorted.entries()) {
    if (
      !TIME_PATTERN.test(interval.startLocalTime) ||
      !TIME_PATTERN.test(interval.endLocalTime) ||
      interval.startLocalTime >= interval.endLocalTime
    ) {
      throw new DomainError(
        'INVALID_VALUE',
        'An interval needs a valid start before its end.'
      );
    }
...
function assertBlockedTimes(schedule: Schedule): void {
    ...
    for (const value of list) {
      ...
      if (!SLOT_MINUTE_MARKS[kind].includes(minutesOf(value) % 60)) {
        throw new DomainError(
          ...
          `${value} is not on the ${kind} booking grid.`
        );
      }
    }
  }
  ...
  for (const entry of schedule.weeklyAvailability) {
    if (
      ...
      throw new DomainError(
        ...
        'A weekday is invalid or listed twice.'
      );
    }
  ...
  for (const entry of schedule.dateExceptions) {
    if (!isValidLocalDate(entry.date) || dates.has(entry.date)) {
      throw new DomainError(
        ...
        'A date exception is invalid or listed twice.'
      );
    }
    ...
    if (entry.kind === 'extra_open') assertIntervals(entry.intervals);
  }

  assertBlockedTimes(schedule);
}

function blockedSetsOf(schedule: Schedule): Record<BookingKind, Set<string>> {
  return {
...
function marksWithin(
  ...
  for (
    ...
    for (const mark of SLOT_MINUTE_MARKS[kind]) {
      ...
      if (blocked.has(clockText(start))) continue;
      found.push(start);
    }
  }
  return found;
...
export function planSlots(
  ...
  if (!isValidLocalDate(options.startDate)) {
    throw new DomainError(
      'INVALID_VALUE',
      'startDate must be a real YYYY-MM-DD calendar date.'
    );
  }
  ...
  const reservations = new Map(
    existingSlots.map((slot) => [slot.id, slot.reservationId])
  );
 ...
 * 以此判斷該日期不可選。
 */
export function followUpGridTimes(
  schedule: Schedule,
  ...
  const times: string[] = [];
  for (const interval of effectiveIntervals(schedule, date as string)) {
    for (const start of marksWithin(
      interval,
      'follow_up',
      blocked,
 ...
 */
export function scheduleImpact<T extends ScheduledAppointmentSnapshot>(
  appointments: readonly T[],
  candidateSlots: readonly SlotSnapshot[]
): T[] {
 ...
 * 的 API 不會各有一套版本規則。
 */
export function assertScheduleVersionMatches(
  expectedVersion: number,
  current: SchedulePublicationState
): void {
  if (expectedVersion !== current.publishedVersion) {
    throw new DomainError(
      'SCHEDULE_VERSION_CONFLICT',
      `The schedule moved to version ${current.publishedVersion} while this draft targeted ${expectedVersion}.`
    );
  }
}
 ...
 */
export function planSchedulePublication(
  request: SchedulePublicationRequest,
  current: SchedulePublicationState,
  existingSlots: readonly SlotSnapshot[],
  appointments: readonly ScheduledAppointmentSnapshot[]
): SchedulePublicationPlan {
  ...
  if (orphaned.length > 0) {
    throw new DomainError(
      ...
      `Publishing would leave ${orphaned.length} open appointment(s) without a slot.`
    );
  }
