...
3. 禁止在 Pull Request、Issue、測試 fixture、截圖或日誌使用真實病患、薪資、日曆或服務帳號資料。安全弱點報告走 [SECURITY.md](SECURITY.md)，不得在公開頻道貼上憑證、祕密或可被利用的敏感細節。
...
10. **每一個階段完成時，必須在 `docs/reviews/` 留下日期化的交接紀錄，並登記到
    ...
    **未處理或未解決的事項**、本機環境陷阱，以及下一位接手者的第一步。模板見
    ...
    每一道關卡必須明列 `PASS`、`FAIL`、`NOT_RUN` 或 `UNAVAILABLE`；後兩者要寫原因，
    不得為了填數字而在唯讀／未授權工作中擅自執行。另須列出稽核涵蓋與未涵蓋範圍、
    產物名稱與 SHA-256、未解 finding 與 owner、下一個可被核准的 Roadmap ID。不可攜的
    本機絕對路徑不得是唯一交付位置；至少保留一份 repository 內、可由 GitHub 取得的
    Markdown。文件自身所在 commit 無法自我引用 hash 時，應明說並以 `git log -- <path>`
    作查找方式，不可填入會因 amend 失效的假 hash。
11. 測試**不得依賴執行當下的時鐘或營業狀態**。需要預約、時段或日期的測試，必須
    把資料錨定到可由狀態推導的固定基準（例如最早時段所在的那一天），不可依賴
    「現在之後的第一個可選時段」之類的相對值。時鐘相依的測試會讓 CI 在夜間或
    休診日整批失敗、白天卻完全正常——那是最難察覺的一種假綠燈。日期一律以
    `Asia/Taipei` 計算，不可直接對 UTC 字串取前十碼。
