...
| 2026-08-11 | **The dependency audit gate is red, and it is not yours.** `SCM-006`: at baseline `cf597af`, `pnpm audit --audit-level high` reported 10 advisories including one high — `nanoid 3.3.16`, reached through `postcss@8.5.20` (`GHSA-2v37-7h3g-55p8`, patched `>=3.3.17`). `Verification evidence` aggregates it, so the branch cannot merge. `pnpm-lock.yaml` still pins both versions, so the condition is unchanged | It does not authorise lowering `--audit-level`, registering an exception or dismissing an alert to clear it. The fix is `SCM-R05` (raise `nanoid`), which is the recorded prerequisite for `SCM-R01`. Until then, report this gate as a pre-existing `FAIL` with its ID rather than as caused by your change |
