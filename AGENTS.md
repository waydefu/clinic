...
| 2026-07-31 | The CodeQL workflow was replaced by a Semgrep CE workflow (`.github/workflows/sast.yml`, `scripts/generate-sast-evidence.mjs`, `security/semgrep/`) because a private personal repository cannot upload code-scanning results | **SEC-02 policy was approved on 2026-08-01.** A 2026-08-11 static audit found that the sole required `Verification evidence` job did not depend on the separate SAST workflow; `SCM-R01` closed that gap on 2026-08-18 — see the row below, and stop describing enforcement as pending. What this still does **not** authorise is unchanged: never describe Semgrep CE as CodeQL-equivalent, and never claim GitHub code-scanning upload works here |
...
| 2026-08-18 | **`SCM-R01` is complete and same-commit Semgrep CE SAST is merge-blocking.** The scan implementation moved to `.github/workflows/sast-scan.yml` (`workflow_call`); `verify.yml` calls it as the `sast` job in the same run and `Verification evidence` now aggregates five required jobs, not four. `sast.yml` remains the weekly-schedule and manual wrapper over the same implementation. Proven behaviourally, not textually: PR #18 (`373757c`, candidate `351e4034`, run `32100761005`) green on all eleven jobs with Semgrep at 0 findings; the intentional-failure PR #19 (candidate `d49330c8`, run `32101192719`) went red on `sast` **and** on `Verification evidence` with every other job green, `mergeStateStatus=BLOCKED`, and was closed unmerged. No branch-protection change was needed — `Verification evidence` is still the single required context | It does not make Semgrep CE equivalent to CodeQL cross-file analysis, does not enable code-scanning upload, and does not touch `SCM-R02` or `SCM-R04`. It also does not remove the D-013 administrator bypass: `enforce_admins` is still false, so a red gate still stops only the protected path |
...
| SAST workflow, Semgrep rules or evidence generation | `.github/workflows/sast-scan.yml` (the implementation) + `.github/workflows/sast.yml` (schedule/manual wrapper) + `security/semgrep/` | Rule fixtures are intentionally unsafe and ESLint-excluded. Since `SCM-R01` (2026-08-18) the same-commit result **is** merge-blocking through `Verification evidence`, so a change here changes the merge gate: keep the scan in the reusable workflow, keep `evidence` depending on `sast`, and re-prove it with an intentional-failure PR rather than by reading the YAML. Still never describe it as CodeQL-equivalent |
